...
const ChatContainer = ({selectedUser, setSelectedUser}) => {
  ...
  return selectedUser ? (
    <div className='h-full overflow-scrol relative backdrop-blur-lg'>
      {/*---- header area */}
      ...
      pb-6'>
        {messagesDummyData.map((msg, index)=>(
          <div key={index} className={`flex item-end gap-2 justify-end ${msg.senderId !== '680f50e4f10f3cd28382ecf9' && 'flex-row-reverse'}`}>
            {msg.image ? (
              <img className='max-w-[230px] border border-gray-700 rounded-lg overflow-hidden mb-8 ' src={assets.img1} alt='' />
            ):(
    ...
    <div className='flex flex-col item-center justify-center gap-2 text-gray-500 
